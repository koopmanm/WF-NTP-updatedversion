...
def extract_amplitude(x, smooth_y):
	# Find extrema
	ex = (np.diff(np.sign(np.diff(smooth_y))).nonzero()[0] + 1)
	if len(ex)>=2 and ex[0]==0:
		ex = ex[1:]
	...
	while index<len(bend_magnitude):
		...
		if dist<bend_threshold:
			...
			prev_index = index
			index += 1
			extrema_amplitude = np.abs(smooth_y[ex] - np.mean(smooth_y))
			extrema.append(extrema_amplitude)
	bend_times = bend_times[idx==1]
	if not extrema:
                extrema_list.append(0)
        else:
                extrema_list.append(100*np.mean(extrema))
        extrema = []
	return extrema_list
...
def extract_data(track):
	...
	for pi,p in reversed(list(enumerate(particles))):
		...
		if sum(idx)>0:
			...
			round_ratio.append(roundness)
                        ...
                        length = 0.001/len(idx)
                        smooth_y = np.arange(0.991,0.992,length)
		...
		# Bend amplitude (average position)
		bend_amplitude = 100 * np.mean(np.abs(smooth_y - np.mean(smooth_y)))
		bend_amplitudes.append(bend_amplitude)

		# Extrema amplitude (average amplitude per bend)
  		extrema_list = extract_amplitude(x, smooth_y)
		extrema_amplitudes.append(extrema_list)
	...
	# Appended lists need to be reversed to same order as particles
        bends, velocites, areas, bend_amplitudes, \
			extrema_amplitudes, max_speed, move_per_bends, round_ratio = map(lambda x: list(reversed(x)), [ 
				bends, velocites, areas, bend_amplitudes, \
				extrema_amplitudes, max_speed, move_per_bends, round_ratio])
	...
	for i,p in enumerate(particles):
        ...
        region, bends, bpm, appears_in, particles, velocites, areas, bend_amplitudes, \
                        move_per_bends, max_speed, extrema_amplitudes, round_ratio = map(np.array, [region, bends, bpm, appears_in, particles,
                                        velocites, areas, bend_amplitudes, move_per_bends, max_speed, extrema_amplitudes, round_ratio])


	return region, region_particles, bends, particles, velocites, areas, \
		bend_amplitudes, move_per_bends, bpm, appears_in, max_speed, extrema_amplitudes, spurious_worms, original_particles, list_number, frames, round_ratio
...
def write_stats(f, stats, dead_stats=True, prepend=''):
	...
	f.write(prepend+'Bend amplitude mean: %.6f\n'%s['extrema_amplitude_mean'])
	f.write(prepend+'Bend amplitude SD: %.6f\n'%s['extrema_amplitude_std'])
	f.write(prepend+'Bend amplitude SEM: %.6f\n'%s['extrema_amplitude_mean_std'])
...
def write_raw_data(f, bpm, bend_amplitudes, velocites,
			areas, move_per_bends, appears_in, region, extrema_amplitudes, max_speed, round_ratio):
	living = idx = np.logical_not((np.logical_and(bpm<=maximum_bpm,
				velocites<=maximum_velocity)))

	f.write('Raw data:\n')
	f.write('Particle    BPM    Bend Measure    Speed    Dist per bend    Area    Appears in frames    Living    Region   Max speed    Bend amplitude   Round_ratio \n')
	f.write('\n'\
	.join(['%i;%.6f;%.6f;%.6f;%s;%.6f;%i;%i;%s;%.6f;%.6f;%.6f'
					%(i,bpm[i],bend_amplitudes[i],velocites[i],
					('nan' if np.isnan(move_per_bends[i]) else
							 '%.6f'%move_per_bends[i]),
					areas[i], appears_in[i], living[i],
					region[i], extrema_amplitudes[i], max_speed[i], round_ratio[i]) for i in xrange(len(bpm))]))
	f.write('\n\n')
...
def statistics(bends, particles, velocites,	areas, bend_amplitudes,
				move_per_bends, bpm, appears_in, max_speed, extrema_amplitudes, track, original_particles, spurious_worms, frames, list_number, round_ratio):
	P = track['particle']
	T = track['frame']
	...
	extrema_amplitude_mean, extrema_amplitude_std = mean_std(extrema_amplitudes, appears_in)
	extrema_amplitude_mean_std = area_std/np.sqrt(max_number_worms_present)

	max_speed_mean, max_speed_std = mean_std(max_speed, appears_in)
	max_speed_mean_std = area_std/np.sqrt(max_number_worms_present)
	...
	stats = {	'max_number_worms_present' : max_number_worms_present,
			...
			'area_mean_std': area_mean_std,
                        'extrema_amplitude_mean': extrema_amplitude_mean,
                        'extrema_amplitude_std': extrema_amplitude_std,
                        'extrema_amplitude_mean_std': extrema_amplitude_mean_std,
...
def write_results_file(region, region_particles, bends, particles, velocites,
	areas, bend_amplitudes, move_per_bends, bpm, appears_in, max_speed, extrema_amplitudes, track, original_particles, spurious_worms, frames, list_number, round_ratio):
	''' Input:
	...
	stats = statistics(bends[idx], particles[idx], velocites[idx],
						...
						bpm[idx], appears_in[idx], max_speed[idx], extrema_amplitudes[idx], track, original_particles, spurious_worms, frames, list_number, round_ratio[idx])
	write_stats(f, stats, dead_stats=False, prepend='Living ')

	# Raw stats
	f.write('---------------------------------\n\n')
	write_raw_data(f, bpm, bend_amplitudes, velocites,
					areas, move_per_bends, appears_in, region, max_speed, extrema_amplitudes, round_ratio)
	...
	if len(regions)>1:
		for reg in regions:
			...
			#stats = statistics(bends[idx], particles[idx], velocites[idx],
							...
							#appears_in[idx], max_speed[idx], extrema_amplitudes[idx], track, original_particles, spurious_worms, frames, list_number)
			#write_stats(f, stats, dead_stats=True)
...
if __name__ == '__main__':
	...
	region, region_particles, bends, particles, velocites, areas, \
		bend_amplitudes, move_per_bends, bpm, appears_in, max_speed, extrema_amplitudes, spurious_worms, original_particles, list_number, frames, round_ratio = extract_data(track)
	...
	write_results_file(region, region_particles, bends, particles,
		velocites, areas, bend_amplitudes, move_per_bends, bpm,
			appears_in, max_speed, extrema_amplitudes, track, original_particles, spurious_worms, list_number, frames, round_ratio)
