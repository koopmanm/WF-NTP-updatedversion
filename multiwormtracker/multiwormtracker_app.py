...
class MainApplication(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.logframe = tkinter.ttk.Frame(canvas)

        logscrollbar = tkinter.ttk.Scrollbar(logframe, orient="vertical",
        ...
        self.consoleframe = tkinter.ttk.Frame(canvas2)
        ...
        self.btnframe = tkinter.ttk.Frame(canvas3)

        start_btn = tkinter.ttk.Button(btnframe, text="Start", width=22,
                                       command=self.start_all)
        start_btn.pack(padx=5, side="left")
        addjob_btn = tkinter.ttk.Button(btnframe, text="Add job", width=22,
                                        command=self.add_job_dialog)
        addjob_btn.pack(padx=5, side="left")
        load_btn = tkinter.ttk.Button(btnframe, text="Load job", width=22,
                                      command=self.load_job)
        load_btn.pack(padx=5, side="left")
        load_btn = tkinter.ttk.Button(btnframe, text="Utilities", width=22,
                                      command=self.utils)
        load_btn.pack(padx=5, side="left")
        exit_btn = tkinter.ttk.Button(btnframe, text="Exit", width=22,
                                      command=self.exit)
    ...
    def add_job(self, job):
        ...
        for videoname in videonames:
            ...
            thisframe = tkinter.ttk.Frame(self.logframe, width=300, height=300)
            thisframe.grid(row=i, column=0, sticky='w')

            deletebtn = tkinter.ttk.Button(thisframe, text='X', width=2,
                                           command=partial(self.delete_job, i))
            deletebtn.pack(side='left')
            videobtn = tkinter.ttk.Button(thisframe, text=short_videoname, width=30,
                                          command=partial(self.edit_job, i))
            videobtn.pack(side='left')

            examplebtn = tkinter.ttk.Button(thisframe, text="Example", width=9,
                                            command=partial(self.example_output, i))
            examplebtn.pack(side='left')

            jobinfo = tkinter.ttk.Label(thisframe, text=short_outputname)
            jobinfo.pack(side='left')
            progressbar = tkinter.ttk.Progressbar(thisframe, orient='horizontal',
...
class Utils(tk.Toplevel):
    def __init__(self, parent, *args, **kwargs):
        ...
        tkinter.ttk.Button(self, text="Plot path", width=30,
                           command=self.plotpath).grid(row=0, padx=20, pady=10)

        tkinter.ttk.Button(self, text="Export to tsv", width=30,
...
class ToTsv(tk.Toplevel):
    def __init__(self, parent, *args, **kwargs):
        ...
        tkinter.ttk.Button(btnframe, text='Add files', width=17,
                           command=self.add).grid(row=0, column=0, pady=5)
        tkinter.ttk.Button(btnframe, text='Add folder', width=17,
                           command=self.addrecursive).grid(row=0, column=1, padx=5, pady=5)
        tkinter.ttk.Button(btnframe, text='Export', width=17,
                           command=self.export).grid(row=0, column=2, padx=0, pady=5)
        cancel_btn = tkinter.ttk.Button(btnframe, width=17, text="Close",
    ...
    def add_filesnames(self, filenames):
        for f in filenames:
            ...
            tkinter.ttk.Label(thisframe, text=f[-70:]).pack()
            self.index += 1

    def add(self):
        filenames = tkinter.filedialog.askopenfilenames(
            title='Locate results.txt files', filetypes=[("results.txt files", "*.txt")])
        self.add_filesnames(filenames)

    def addrecursive(self):
        folder = tkinter.filedialog.askdirectory()
        if folder and tkinter.messagebox.askquestion("Warning",
...
class Fingerprint(tk.Toplevel):
    def __init__(self, parent, *args, **kwargs):
        ...
        tkinter.ttk.Button(btnframe, text='Add files', width=17,
                           command=self.add).grid(row=0, column=0, pady=5)
        tkinter.ttk.Button(btnframe, text='Make graph', width=17,
    ...
    def export(self):
        ...
        if small_figure:
            radar_chart.set_publish()  # update parameters
        radar_chart.plot_results(input_files, None, plot_section=section_to_plot, corners=variables_to_plot,
                                 color_palette=color, legend_labels=legend_labels, save='test.png')
...
class AddJob(tk.Toplevel):
    def __init__(self, parent, *args, **kwargs):
        ...
        videoframe = tkinter.ttk.LabelFrame(self, text="Video")
        videoframe.grid(row=0, sticky='w')

        videonameframe = tkinter.ttk.Frame(videoframe)
        videonameframe.grid(row=0)
        self.videoname = tkinter.ttk.Entry(
            videonameframe, width=80, state='readonly')

        self.videoname.grid(row=0, padx=10)
        video_btn = tkinter.ttk.Button(videonameframe, text="Browse",
                                       command=self.find_video)
        video_btn.grid(row=0, column=1, padx=5)
        self.videoinfo = tkinter.ttk.Label(
            videonameframe, text='No video chosen.')
        self.videoinfo.grid(row=1, sticky='w', padx=10)

        videocutframe = tkinter.ttk.Frame(videoframe)
        videocutframe.grid(row=1, sticky='w')

        start_frame_label = tkinter.ttk.LabelFrame(
            videocutframe, text="Start frame")
        start_frame_label.grid(row=2, sticky='w', padx=10)
        self.start_frame = tkinter.ttk.Entry(start_frame_label, width=15)
        self.start_frame.insert(0, '0')
        self.start_frame.pack(side='left', padx=3)

        use_frame_label = tkinter.ttk.LabelFrame(
            videocutframe, text="Use frames")
        use_frame_label.grid(row=2, column=1, sticky='w', padx=10)
        self.use_frame = tkinter.ttk.Entry(use_frame_label, width=15)
        self.use_frame.pack(side='left', padx=3)

        fps_label = tkinter.ttk.LabelFrame(videocutframe, text="FPS")
        fps_label.grid(row=2, column=2, sticky='w', padx=10)
        self.fps = tkinter.ttk.Entry(fps_label, width=15)
        self.fps.insert(0, '20')
        self.fps.pack(side='left', padx=3)

        px_to_mm_label = tkinter.ttk.LabelFrame(
            videocutframe, text="px to mm factor")
        px_to_mm_label.grid(row=2, column=3, sticky='w', padx=10)
        self.px_to_mm = tkinter.ttk.Entry(px_to_mm_label, width=15)
        self.px_to_mm.insert(0, '0.040')
        self.px_to_mm.pack(side='left', padx=3)

        darkfield_label = tkinter.ttk.LabelFrame(
            videocutframe, text="Darkfield")
        ...
        ###### LOCATING SECTION #######
        locate_frame = tkinter.ttk.LabelFrame(self, text="Locating")
        locate_frame.grid(row=1, sticky='w', pady=padframes)

        methodframe = tkinter.ttk.Frame(locate_frame)
        methodframe.grid(row=0)

        method_label = tkinter.ttk.LabelFrame(methodframe, text="Method")
        method_label.grid(row=0, column=0, sticky='w', padx=10)
        self.method = tkinter.ttk.Combobox(method_label, values=(
            'Keep Dead', 'Z-filtering'), state='readonly')
        self.method.pack(side='left', padx=3)
        self.method.current(0)

        z_use_label = tkinter.ttk.LabelFrame(methodframe, text="Z use images")
        z_use_label.grid(row=0, column=1, sticky='w', padx=10)
        self.z_use = tkinter.ttk.Entry(z_use_label, width=15)
        self.z_use.insert(0, '100')
        self.z_use.configure(state="disabled")
        self.z_use.pack(side='left', padx=3)

        z_padding_label = tkinter.ttk.LabelFrame(methodframe, text="Z padding")
        z_padding_label.grid(row=0, column=2, sticky='w', padx=10)
        self.z_padding = tkinter.ttk.Entry(z_padding_label, width=15)
        self.z_padding.insert(0, '5')
        self.z_padding.configure(state="disabled")
        self.z_padding.pack(side='left', padx=3)

        std_px_label = tkinter.ttk.LabelFrame(methodframe, text="Std pixels")
        std_px_label.grid(row=0, column=3, sticky='w', padx=10)
        self.std_px = tkinter.ttk.Entry(std_px_label, width=15)
        self.std_px.insert(0, '64')
        self.std_px.pack(side='left', padx=3)

        threshold_label = tkinter.ttk.LabelFrame(
            methodframe, text="Threshold (0-255)")
        threshold_label.grid(row=1, column=1, sticky='w', padx=10, pady=10)
        self.threshold = tkinter.ttk.Entry(threshold_label, width=15)
        self.threshold.insert(0, '9')
        self.threshold.pack(side='left', padx=3)

        opening_label = tkinter.ttk.LabelFrame(methodframe, text="Opening")
        opening_label.grid(row=1, column=2, sticky='w', padx=10, pady=10)
        self.opening = tkinter.ttk.Entry(opening_label, width=15)
        self.opening.insert(0, '1')
        self.opening.pack(side='left', padx=3)

        closing_label = tkinter.ttk.LabelFrame(methodframe, text="Closing")
        closing_label.grid(row=1, column=3, sticky='w', padx=10, pady=10)
        self.closing = tkinter.ttk.Entry(closing_label, width=15)
        self.closing.insert(0, '3')
        self.closing.pack(side='left', padx=3)

        skeletonize_label = tkinter.ttk.LabelFrame(
        ...
        prune_size_label = tkinter.ttk.LabelFrame(
            methodframe, text="Prune size")
        prune_size_label.grid(row=2, column=2, sticky='w', padx=10)
        self.prune_size = tkinter.ttk.Entry(prune_size_label, width=15)
        self.prune_size.insert(0, '0')
        self.prune_size.pack(side='left', padx=3)

        do_full_prune_label = tkinter.ttk.LabelFrame(methodframe,
                                                     text="Full prune")
        ...
        filter_frame = tkinter.ttk.LabelFrame(self, text="Filtering")
        filter_frame.grid(row=2, sticky='w')

        minsize_label = tkinter.ttk.LabelFrame(filter_frame,
                                               text="Minimum size (px)")
        minsize_label.grid(row=0, column=0, sticky='w', padx=10)
        self.minsize = tkinter.ttk.Entry(minsize_label, width=15)
        self.minsize.insert(0, '25')
        self.minsize.pack(side='left', padx=3)

        maxsize_label = tkinter.ttk.LabelFrame(filter_frame,
                                               text="Maximum size (px)")
        maxsize_label.grid(row=0, column=1, sticky='w', padx=10)
        self.maxsize = tkinter.ttk.Entry(maxsize_label, width=15)
        self.maxsize.insert(0, '120')
        self.maxsize.pack(side='left', padx=3)

        minimum_ecc_label = tkinter.ttk.LabelFrame(filter_frame,
                                                   text="Worm-like (0-1)")
        minimum_ecc_label.grid(row=0, column=2, sticky='w', padx=10)
        self.minimum_ecc = tkinter.ttk.Entry(minimum_ecc_label, width=15)
        ...
        cut_off_frame = tkinter.ttk.LabelFrame(
        ...
        cutoff_filter_label = tkinter.ttk.LabelFrame(
        ...
        Bends_max_label = tkinter.ttk.LabelFrame(cut_off_frame,
                                                 text="Max Bends")
        Bends_max_label.grid(row=0, column=5, sticky='w', padx=5)
        self.Bends_max = tkinter.ttk.Entry(Bends_max_label, width=7)
        self.Bends_max.insert(0, '20')
        self.Bends_max.pack(side='left', padx=3)

        Speed_max_label = tkinter.ttk.LabelFrame(cut_off_frame,
                                                 text="Max Speed")
        Speed_max_label.grid(row=0, column=6, sticky='w', padx=5)
        self.Speed_max = tkinter.ttk.Entry(Speed_max_label, width=7)
        ...
        roi_frame = tkinter.ttk.LabelFrame(self, text="Region of interests")
        roi_frame.grid(row=7, sticky='w')

        roi_btn_add = tkinter.ttk.Button(roi_frame, text="Add new",
                                         command=self.add_roi)
        roi_btn_add.pack(side='left', padx=15)

        self.rois = tkinter.ttk.Combobox(roi_frame, state='disabled')
        self.rois.pack(side='left', padx=3)
        self.rois['values'] = []

        self.roi_btn_show = tkinter.ttk.Button(roi_frame, text="Show",
                                               command=self.show_roi, state='disabled')
        self.roi_btn_show.pack(side='left', padx=5)

        self.roi_btn_edit = tkinter.ttk.Button(roi_frame, text="Redraw",
                                               command=self.edit_roi, state='disabled')
        self.roi_btn_edit.pack(side='left', padx=5)

        self.roi_btn_del = tkinter.ttk.Button(roi_frame, text="Delete",
        ...
        output_frame = tkinter.ttk.LabelFrame(self, text="Output")
        output_frame.grid(row=8, sticky='w')

        outputdirframe = tkinter.ttk.Frame(output_frame)
        outputdirframe.grid(row=0)
        self.outputname = tkinter.ttk.Entry(
            outputdirframe, width=80, state='readonly')

        self.outputname.grid(row=0, padx=10)
        output_btn = tkinter.ttk.Button(outputdirframe, text="Browse",
                                        command=self.find_outputfolder)
        output_btn.grid(row=0, column=1, padx=5)

        outputinfo_frame = tkinter.ttk.Frame(output_frame)
        outputinfo_frame.grid(row=1, sticky='w')

        outputframes_label = tkinter.ttk.LabelFrame(outputinfo_frame,
                                                    text="Output frames")
        outputframes_label.grid(row=2, sticky='w', padx=10)
        self.outputframes = tkinter.ttk.Entry(outputframes_label, width=15)
        self.outputframes.insert(0, '0')
        self.outputframes.pack(side='left', padx=3)

        font_size = tkinter.ttk.LabelFrame(outputinfo_frame, text="Font size")
        font_size.grid(row=2, column=1, sticky='w', padx=10)
        self.font_size = tkinter.ttk.Entry(font_size, width=15)
        self.font_size.insert(0, '8')
        self.font_size.pack(side='left', padx=3)

        ######### Buttons #########
        add_job_btn_frame = tkinter.ttk.Frame(self)
        add_job_btn_frame.grid(row=9, sticky='W')
        if editing:
            add_job_btn = tkinter.ttk.Button(add_job_btn_frame, text="Confirm edits",
                                             command=self.add_job)
        else:
            add_job_btn = tkinter.ttk.Button(add_job_btn_frame, text="Add job",
                                             command=self.add_job)
        add_job_btn.pack(side='left', padx=5)
        cancel_btn = tkinter.ttk.Button(add_job_btn_frame, text="Cancel",
        ...
        if editing:
            ...
            self.rois['values'] = list(edit_job['regions'].keys())
            if len(edit_job['regions']) > 0:
                self.rois.set(list(edit_job['regions'].keys())[0])
    ...
    def add_job(self):
        ...
        if not add(job, 'fps', self.fps, float):
            return
        if not add(job, 'px_to_mm', self.px_to_mm, float):
            return
        if not add(job, 'darkfield', self.darkfield, bool):
            return
        if not add(job, 'extra_filter', self.extra_filter, bool):
            return  # new
        ...
        if not add(job, 'std_px', self.std_px, int):
            return
        if not add(job, 'threshold', self.threshold, int):
            return
        if not add(job, 'opening', self.opening, int):
            return
        if not add(job, 'closing', self.closing, int):
            return
        if not add(job, 'skeletonize', self.skeletonize, bool):
            return
        if not add(job, 'do_full_prune', self.do_full_prune, bool):
            return
        if not add(job, 'prune_size', self.prune_size, int):
            return
        if not add(job, 'lower', self.lower, int):
            return  # new
        if not add(job, 'upper', self.upper, int):
            return  # new
        if not add(job, 'Bends_max', self.Bends_max, float):
            return  # new
        ...
        if not add(job, 'minimum_ecc', self.minimum_ecc, float):
            return
        if not add(job, 'font_size', self.font_size, int):
            return
            ...
            if x is not None:
                ...
                if len(self.xx) >= 3:
                    if poly[0] != 1:
                        poly[0].pop(0).remove()
                    poly[0] = self.ax.plot(
                        [self.xx[0], self.xx[-1]], [self.yy[0], self.yy[-1]], '--b')
                self.ax.axis(axes)
                self.canvas.draw()
